...
# Primero recibe 4 bytes para saber cuánto viene
     ...
     # Primero recibe el tamaño (4 bytes)
        ...
        bob_bases = np.random.choice(['Z', 'W', 'V'], size=num_qubits)
        ...
        serialized_result = b"".join(bytes([bit]) for bit in bob_result)

        client_socket1.sendall(serialized_bases + b'|' + serialized_result)
