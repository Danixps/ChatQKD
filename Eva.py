...
ruta_bb84 = os.path.join(base_dir, "BB84", "eavesdropper.py")
ruta_bbm92 =  os.path.join(base_dir, "BBM92", "eavesdropper.py")
ruta_E91 =  os.path.join(base_dir, "E91", "eavesdropper.py")
ruta_SARG04 = os.path.join(base_dir, "SARG04", "eavesdropper.py")
...
def ejecutar_protocolo():
    ...
    while reintento < max_reintentos:
            try:
                ...
                s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
                s.connect(("localhost", 59006))
                conectado = True
                break
            except socket.error as e:
                print(f"[ERROR] No se pudo conectar a :59001. Reintentando... ({reintento + 1}/{max_reintentos})")
                time.sleep(5)
                reintento += 1
    if not conectado:
